...
# - say_hello naam ka ek function define kiya jo ek parameter leta hai jo ek name parameter leta hai aur uska use kar ke kuch print karta hai
# - fir humne function call kiya aur function call karne ke time ek argument diya jiski value "Aatif" thi
# - jab yeh function call hota hai toh jo humne string, "Aatif" argument diya hai uski value name parameter mein jaati hai aur function ka andar name variable mein \
#   chali jati hai jiska use karke humne print statement likhi hui hai.
# Note: Yeh thoda sa mushkil concept, agar bahot ache se samajh nahi aaya, toh ek aur baar padh ke aur dusre examples samajh ke zaroor aa jayega ;-)
...
# Iske baad humne "say_bye" function ko call kiya aur argument mein ek string, "Drishti" diya. Jab function call hua toh iss argument ki value function ke name parameter mein jaati
#   jahan se woh function andr name variable mein aa jati hai. Hum jis naam se function parameters likhte hain, ussi naam se hum unki value function ke andar se same naam ke variable
#   se le sakte hain. Jaise yahan name wale parameter ki value name variable mein hai jo humne print command ke saath use kiya hai.
...
# Humne pehlie baar function ko call karte samay, add_numbers(120, 50) usko do integer parameter diye hai. Yahan parameters ka kram / order important hai. Iss function call mein:
...
# Hum jab say_hello_language("Rishabh", "punjabi") call karte hain toh:
# - pehle parameter, name mein "Rishabh" string jata hai aur dusre parameter, language mein punjabi jaata hai.
# - fir humara program if-elif-else ka use kar ke dekhta hai language ki value hai aur uske hisaab se sahi language mein print kar deta hai
# Isi tareeke se upar likhi hui saari function calls mein yehi hota hai


# 4 ARGUMENTS ke saath ek aur FUNCTION ka example
# Chalane se pehle isko padh ke output ko sochne ki koshish karo. Fir chala ke dekho ki aapne sahi output sochi thi
...
# def waali line mein humne 4 parameters likhe hain comma laga laga ke.
# Function call karte samay jis kram / order mein humne parameters likhe hain def waali line mein, usse kram / order mein arguments ki value parameters mein jaati hai.
# say_hello_people("Imitiyaz", "Rishabh", "Rahul", "Vidya") mein:
